...
def mix_images(image1, image2, channel1, channel2):
    ...
    out = np.concatenate((out1[:,:out1.shape[1]//2],out2[:,out2.shape[1]//2:]),axis = 1)
    ### END YOUR CODE
...
def mix_quadrants(image):
    ...
    ### YOUR CODE HERE
    pass
    out1 = rgb_exclusion(image[:image.shape[0]//2,:image.shape[1]//2], 'R')
    out2 = dim_image(image[:image.shape[0]//2,image.shape[1]//2:])
    out3 = np.power(image[image.shape[0]//2:,:image.shape[1]//2],0.5)
    out4 = rgb_exclusion(image[image.shape[0]//2:,image.shape[1]//2:], 'R')
    
    out = np.concatenate((np.concatenate((out1,out3),axis = 0),np.concatenate((out2,out4),axis = 0)),axis = 1)
    ### END YOUR CODE
